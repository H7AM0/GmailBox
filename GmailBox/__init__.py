...
class GmailBox:
    ...
    def new_email(self) -> new_emailResult:
        ...
        r = make_request('https://smailpro.com/temp-gmail', method='get', headers=self.headers, response_type='cookies')
        self.headers["x-xsrf-token"] = unquote(r['XSRF-TOKEN'])
        self.headers["x-g-token"] = reCaptcha_response
        if not self.rapidapi:
            req = make_request('https://smailpro.com/js/chunks/smailpro_v2_email.js', method='get', headers=self.headers, response_type='text')
        ...
        if not self.key:
            ...
            response = make_request('https://smailpro.com/app/key', method='post', headers=self.headers, json=json_data, response_type='json')
            self.key = response['items']

        req = make_request(f'https://public-sonjj.p.rapidapi.com/email/gm/get?key={self.key}&rapidapi-key={self.rapidapi}&domain=gmail.com&username=random&server=server-1&type=alias', method='get', headers=self.headers, response_type='json')
    ...
    def inbox(self, email: Optional[str] = None, limit: int = None) -> List[inboxResult]:
        ...
        response = make_request('https://smailpro.com/app/key', method='post', headers=self.headers, json=json_data, response_type='json')
        self.key = response['items']
        response = make_request(f'https://api.sonjj.com/email/gm/check?key={self.key}&rapidapi-key={self.rapidapi}&email={email}&timestamp={self.timestamp}', method='get', headers=self.headers, response_type='json')
        messages = []
        for item in response["items"]:
            ...
            response = make_request('https://smailpro.com/app/key', method='post', headers=self.headers, json=json_data, response_type='json')
            self.key = response['items']
            response = make_request(f'https://api.sonjj.com/email/gm/read?key={self.key}&email={email}&message_id={message_id}', method='get', headers=self.headers, response_type='json')
